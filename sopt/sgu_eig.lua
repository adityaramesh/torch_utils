...
function sopt.sgu_eig(func, x, config, state)
	...
	elseif mom_type == sopt.nag then
		if not state.temp then
			local loss, grad = func(x)
			local eig, eigvec = estimate_max_eigenvalue(func, x, grad)

			state.temp = torch.Tensor():typeAs(x):resizeAs(x):
				copy(grad):mul(-cur_lr / math.abs(eig))
			x:add(state.temp)
			...
			local delta = eigvec:dot(grad)
			local theta_2 = math.acos(delta / grad:norm())
		...
		state.temp:add(-1, x):add(-cur_lr / math.abs(eig), grad)
		x:add(state.temp)

		local descent = state.temp:dot(grad)
		local theta_1 = math.acos(descent / (state.temp:norm() * grad:norm()))
